...
## Finite Element Method for non-computery people
In glacier modeling, we need to solve a series of complex Partial Differential Equations (PDEs) to accurately describe how ice flows. If you've ever taken a PDE class, you know how much this sucks to do. In our particular case, we're trying to solve simplifications of the [Navier-Stokes equations](https://navier-stokes.org/navier-stokes-derivation/), a [Millennium Prize Problem](https://www.claymath.org/millennium/navier-stokes-equation/) aka nobody has solved them yet and you'll get a million dollars if you do. Needless to say, we need to significantly simplify things to get them to run on any reasonable timescale for any fluid flow application. Luckily for us, these approximations do a decent job at replicating observations, but they still take a ton of computer power to solve. 

In real life, materials are infinite continuums with infinite degrees of freedom to account for in the PDEs. It is not possible to analytically solve these PDEs, so we use FEM to break down the domain into smaller sub elements 

I found [this video](https://www.youtube.com/watch?v=hVleTL6CeKw) overview to be very helpful in understanding what is going on in a finite element model such as icepack.

### Navier-Stokes equations summarized for glaciers with the least amount of math possible
The Navier-Stokes equations at their simplest are an expansion of Newton's Laws. [This video](https://www.youtube.com/watch?v=Ra7aQlenTb8) provides a very good overview without getting too deep into the math. The first equation is **Conservation of Mass**, represented as: $\nabla\cdot\vec{u}$. This equation says that the divergence of the velocity is zero, meaning there are no places within the system where mass mysteriously appears or disappears. It also tells us that the material is incompressible. This means that the density of the material does not change in time or space. 

The second part of Navier-Stokes is the **Conservation of Momentum**, which most people will recognize as $\vec{F}=m\vec{a}$. In glaciology, we do a lot of approximating to show that the acceleration side of the equation is negligible compared to the forces, so we treat glacier flow as a balance of forces: $\Sigma \vec{F}=0$

For glacier modeling, there are several key forces to take into account:
- Pressure gradient: this is the force, often called the driving stress, generated by the weight of the ice. The driving stress is typically represented as $\tau=\rho gh\alpha$ where $\alpha$ is the angle of the surface of the glacier. The pressure gradient tells us that if two ice columns next to each other have different heights, the taller column wants to flow in to the shorter column until they are the same height to equalize the pressure of the system
- Viscous forces: how much force is the chunk of ice right next to another chunk exerting on that chunk?
- Any other external forces: This incorporates the force of gravity and any friction from the bed or side walls in our glacier
  
### Function Spaces
In finite element modeling, our model will try to fit partial differential equations (PDEs) across the mesh by subdividing the overarching equations defining our system to simpler algebraic expressions within each finite element (triangles in our case). The 

In icepack we will primarily use Continuous Galerkin (CG) elements to represent function spaces for thickness and velocity. 

velocity/thickness using continuous galerkin elements which is appropriate for elliptic PDEs (it forces matching of the solution at the element boundaries). the damage PDE is a hyperbolic PDE that evolves by advection, so we use discontinuous galerkin elements which allows the field to be discontinuous across the boundaries between elements. Not super surprising I think that it won't work when they are both using one or the other

### Boundary Conditions
#### Neumann Boundary
Sets the values that the derivative of a solution is to take along the boundary of the domain

#### Dirichlet Boundary
Sets the value of a solution along the boundary of the domain. For us, this typically means setting flux in as a constant along the upstream boundary of the glacier, or setting velocity at the side walls to be 0.

#### Robin Boundary
I have not yet seen this used in icepack. The Robin Boundary is setting a linear combination of the function value and derivative value along the domain boundary
